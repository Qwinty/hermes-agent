...
def _adapter():
    adapter = telegram_mod.TelegramAdapter(PlatformConfig(enabled=True, extra={"guest_mode": True}))
...
async def test_guest_update_routes_to_answer_target():
    ...
    update = SimpleNamespace(update_id=7, guest_message=None, api_kwargs={"guest_message": raw})
...
async def test_guest_send_answers_query(monkeypatch):
    adapter = _adapter()
    monkeypatch.setattr(adapter, "_raw_answer_guest_query", AsyncMock(return_value={"inline_message_id": "inline-1"}))
...
async def test_guest_send_edits_existing_inline(monkeypatch):
    ...
    edit = AsyncMock(return_value=telegram_mod.SendResult(success=True, message_id="inline-1"))
