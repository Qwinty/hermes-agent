...
async def test_table_only_uses_legacy_with_default_config():
    """Default config keeps rich delivery opt-in, even for bare tables."""
    config = PlatformConfig(enabled=True, token="fake-token")
    adapter = TelegramAdapter(config)
    bot = MagicMock()
    bot.do_api_request = AsyncMock(return_value=SimpleNamespace(message_id=123))
...
@pytest.mark.asyncio
async def test_dm_topic_resumed_table_send_respects_rich_opt_out():
    """Resumed/synthetic DM-topic table sends stay legacy when rich is disabled."""
    adapter = _make_adapter(extra={"rich_messages": False})
...
async def test_finalize_edit_opt_out_uses_legacy():
    ...
    result = await adapter.edit_message(
        "12345", "555", TABLE_ONLY_CONTENT, finalize=True,
    )
